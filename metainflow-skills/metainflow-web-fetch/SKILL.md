...
├─ 无具体 URL，需搜索 → metainflow search-summary（见 metainflow-web-search skill）
└─ 先搜索再深入 → search-summary 获取 URL（见 metainflow-web-search skill）→ web-crawl 提取详情
```
...
metainflow web-crawl --url <搜索结果中的URL> --instruction "提取项目名称、描述和 Star 数"
```
...
| 错误 | 正确做法 |
|------|----------|
| 无具体 URL 时用 `metainflow web-crawl` | 无 URL 用 `metainflow search-summary`（见 metainflow-web-search skill） |
